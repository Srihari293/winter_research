...
String oscAddress2 = "/EmotiBit/0/HR"; 
int oscPort = 12345;
String arduinoPortName = "COM11"; // Replace with your Arduino's port name
...
void draw() {
  if (dataListEDA.size() > 0 && dataListPPGIR.size() > 0) {
    ...
    float dataHR = dataListHR.get(dataListPPGIR.size() - 1); // get the most recent PPG:IR data point
    sendToArduino(dataEDA, dataHR); // Send EDA data to Arduino
  }
}
